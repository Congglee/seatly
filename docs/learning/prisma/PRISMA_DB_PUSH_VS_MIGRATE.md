...
Trong dự án `seatly`, Prisma schema đang nằm tại `seatly-server/prisma/schema.prisma` và datasource đang dùng `postgresql`. Đây là một bối cảnh rất phù hợp để phân biệt hai cách cập nhật schema database phổ biến trong Prisma:
...
## 9. Áp dụng vào dự án `seatly`

Trong codebase hiện tại, schema ở `seatly-server/prisma/schema.prisma` đã có khá nhiều model và relation như:
