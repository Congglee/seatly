...
- Prisma schema nằm tại `seatly-server/prisma/schema.prisma`
- Dự án đang dùng `generator client` với `provider = "prisma-client-js"`
- Datasource đang dùng `postgresql`
- Package `@prisma/client` và `prisma` đã có trong `seatly-server/package.json`
...
## Áp dụng vào dự án `seatly`

Schema của dự án `seatly` hiện có nhiều model và relation như:
