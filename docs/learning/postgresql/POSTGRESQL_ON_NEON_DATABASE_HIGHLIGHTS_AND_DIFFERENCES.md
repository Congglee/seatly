...
## Bối cảnh trong dự án Seatly

Trong codebase hiện tại:

- Backend `seatly-server` dùng Prisma `^5.16.1` với PostgreSQL datasource tại `seatly-server/prisma/schema.prisma:5`
- Web dùng Next.js `14.2.35` tại `seatly-web/package.json:14`
- File `seatly-server/.env:7` hiện đang trỏ tới PostgreSQL local
...
Hiện tại `seatly-server/prisma/schema.prisma:5` đang có:
...
### Vì sao cấu hình này hợp với Seatly?
...
## Neon phù hợp với Seatly khi nào?

Neon rất hợp nếu bạn định hướng Seatly theo các tiêu chí sau:
