...
import { type CreateOrdersBodyType } from "@/schemas/order.schema";
...
export default function NewOrder() {
  ...
  const form = useForm<AddOrderGuestFormValues>({
    resolver: zodResolver(addOrderGuestSchema),
    defaultValues: {
      name: "",
      tableNumber: 0,
    },
  ...
  const submitOrder = async (values?: AddOrderGuestFormValues) => {
  ...
  return (
    <Sheet open={newOrderSheetOpen} onOpenChange={handleSheetOpenChange}>
      <SheetContent className="scroll w-full space-y-4 overflow-y-auto sm:max-w-2xl">
        <SheetHeader>
          <SheetTitle>Add Order</SheetTitle>
          <SheetDescription>
            Create an order for a new guest or assign dishes to an existing guest.
          </SheetDescription>
        </SheetHeader>

        <Form {...form}>
          <form onSubmit={handleFormSubmit} className="space-y-6 pt-4" noValidate>
                      ...
                      <div className="space-y-1 text-sm">
                        ...
                        {selectedGuest.tableNumber === null && (
                          <p className="text-xs text-destructive">
                            This guest cannot be used because no table is assigned.
                          </p>
                        )}
                      </div>
                    </div>
            ...
            <SubmitButton
              ...
              disabled={isNewGuest ? !canSubmitNewGuest : !canSubmitExistingGuest}
            >
