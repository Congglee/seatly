...
export const getDashboardIndicatorsController = async ({ fromDate, toDate }: DashboardIndicatorQueryParamsType) => {
  ...
  const servingTableNumbers = new Set<number>(
    servingOrders.map((order) => order.tableNumber).filter((tableNumber): tableNumber is number => tableNumber !== null)
  )
  const revenueByDateMap = new Map<string, number>(rangeDateKeys.map((dateKey) => [dateKey, 0]))
  ...
  paidOrders.forEach((order) => {
    ...
    revenueByDateMap.set(dateKey, (revenueByDateMap.get(dateKey) ?? 0) + order.dishSnapshot.price * order.quantity)
